...
- [-] 1. Tag current baseline implementation
...
- [~] 2. Set up advanced backend infrastructure
...
- [ ] 3. Implement FileDispositionInfoEx deletion method
  - [~] 3.1 Implement `deleteWithFileInfo()` using SetFileInformationByHandle
  ...
  - [~] 3.2 Write property test for FileDispositionInfoEx fallback chain
    - **Property 1: Fallback chain completeness**
    - **Validates: Requirements 1.1, 1.2, 1.3**
  
  - [~] 3.3 Write property test for read-only file handling
    - **Property 2: Read-only file handling**
    - **Validates: Requirements 1.4**

- [ ] 4. Implement FILE_FLAG_DELETE_ON_CLOSE deletion method
  - [~] 4.1 Implement `deleteWithDeleteOnClose()` method
    - Open file with DELETE_ON_CLOSE flag
    - Close handle to trigger deletion
    - Handle errors appropriately
    - _Requirements: 1.5_
  
  - [~] 4.2 Write property test for DELETE_ON_CLOSE correctness
    - **Property 3: FILE_FLAG_DELETE_ON_CLOSE correctness**
    - **Validates: Requirements 1.5**

- [ ] 5. Implement NtDeleteFile native API method
  - [~] 5.1 Implement `deleteWithNtAPI()` using NtDeleteFile
  ...
  - [~] 5.2 Implement NT status code translation
    - Create `translateNTStatus()` function
    - Map common NT status codes to readable messages
    - _Requirements: 8.4_
  
  - [~] 5.3 Write property test for NtDeleteFile correctness
    - **Property 4: NtDeleteFile correctness**
    - **Validates: Requirements 1.6, 8.4**

- [~] 6. Checkpoint - Ensure all deletion method tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Implement deletion method selection and fallback
  - [~] 7.1 Implement `SetDeletionMethod()` and method routing
...
- [ ] 8. Implement parallel directory scanning
  - [~] 8.1 Create `ParallelScanner` struct and constructor
    - Define ParallelScanner with worker count and configuration
    - Add `NewParallelScanner()` constructor
    - _Requirements: 3.1, 3.2_
  
  - [~] 8.2 Implement FindFirstFileEx-based scanning
  ...
  - [~] 8.3 Write property test for parallel subdirectory processing
    - **Property 8: Parallel subdirectory processing**
    - **Validates: Requirements 3.2**
  
  - [~] 8.4 Write property test for bottom-up ordering invariant
    - **Property 9: Bottom-up ordering invariant**
    - **Validates: Requirements 3.3**
  
  - [~] 8.5 Write property test for scan fallback correctness
    - **Property 11: Scan fallback correctness**
    - **Validates: Requirements 3.5**

- [ ] 9. Implement UTF-16 pre-conversion
  - [~] 9.1 Add UTF-16 path storage to ScanResult
    - Add `FilesUTF16 []*uint16` field to ScanResult
    - Implement `convertToUTF16()` in scanner
    - Store UTF-16 paths during scan phase
    - _Requirements: 3.4, 5.1_
  
  - [~] 9.2 Modify backend to accept and reuse UTF-16 paths
  ...
  - [~] 9.3 Write property test for UTF-16 pre-conversion
    - **Property 10: UTF-16 pre-conversion completeness**
    - **Validates: Requirements 3.4, 5.1**
  
  - [~] 9.4 Write property test for UTF-16 reuse without re-conversion
    - **Property 15: UTF-16 reuse without re-conversion**
    - **Validates: Requirements 5.2, 5.3**

- [~] 10. Checkpoint - Ensure scanning and UTF-16 tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Implement atomic counters in engine
  - [~] 11.1 Replace mutex-protected counters with atomic operations
...
- [ ] 12. Implement dynamic buffer sizing and worker tuning
  ...
  - [~] 12.3 Update default worker count to NumCPU * 4
    - Change default from NumCPU * 2 to NumCPU * 4
    - _Requirements: 4.2_
  
  - [~] 12.4 Write property test for buffer size calculation
    - **Property 12: Buffer size calculation**
    - **Validates: Requirements 4.3, 5.4**
  
  - [~] 12.5 Write property test for adaptive worker adjustment
    - **Property 13: Adaptive worker adjustment**
    - **Validates: Requirements 4.4**

- [ ] 13. Implement batch memory release
  - [~] 13.1 Add batch processing with memory release
...
- [~] 14. Checkpoint - Ensure engine optimization tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 15. Implement benchmarking mode
  - [~] 15.1 Create BenchmarkConfig and BenchmarkResult structs
    - Define configuration for benchmark runs
    - Define result structure with timing and metrics
    - _Requirements: 6.1, 6.2_
  
  - [~] 15.2 Implement `RunBenchmark()` function
  ...
  - [~] 15.3 Write property test for benchmark isolation
    - **Property 17: Benchmark isolation**
    - **Validates: Requirements 6.5**

- [ ] 16. Add CLI flags for configuration
  - [~] 16.1 Add --workers flag
    - Parse worker count from CLI
    - Pass to engine configuration
    - Validate positive integer
    - _Requirements: 11.1_
  
  - [~] 16.2 Add --buffer-size flag
    - Parse buffer size from CLI
    - Pass to engine configuration
    - Validate positive integer
    - _Requirements: 11.2_
  
  - [~] 16.3 Add --deletion-method flag
  ...
  - [~] 16.4 Add --benchmark flag
    - Enable benchmarking mode
    - Run comparative benchmarks
    - Display results table
    - _Requirements: 6.1_
  
  - [~] 16.5 Implement configuration validation
    - Validate all flag combinations
    - Display error and exit with code 2 for invalid config
    - _Requirements: 11.5_
  
  - [~] 16.6 Write property tests for CLI flag handling
...
- [ ] 20. Add integration tests and performance validation
  - [~] 20.1 Write integration test for performance improvement
    - **Property 5: Optimized throughput improvement**
    - **Validates: Requirements 2.1**
  
  - [~] 20.2 Write integration test for parallel scan performance
    - **Property 6: Parallel scan performance**
    - **Validates: Requirements 2.2**
  
  - [~] 20.3 Write integration test for memory scaling
    - **Property 7: Sub-linear memory scaling**
    - **Validates: Requirements 2.3**

- [ ] 21. Update documentation and build configuration
  - [~] 21.1 Update README with new CLI flags
    - Document --workers, --buffer-size, --deletion-method, --benchmark
    - Add performance optimization section
    - Include benchmark examples
    - _Requirements: 2.4_
  
  - [~] 21.2 Update Makefile with optimization build targets
    - Ensure Windows-specific code compiles correctly
    - Add benchmark target
    - _Requirements: 9.2, 9.3_
  
  - [~] 21.3 Add build tags to all Windows-specific files
    - Ensure `//go:build windows` tags are present
    - Verify generic fallbacks have `//go:build !windows`
    - _Requirements: 9.1_

- [~] 22. Final checkpoint - Run full test suite and benchmarks
